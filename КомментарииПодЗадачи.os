...
Процедура ОбновитьШаблон()
	...
	Если НЕ ПустаяСтрока(СтрДобавить) Тогда
		СтрокаВставки = ТекстДок.КоличествоСтрок()-2; // предполагаем, что блок шаблонов заканчивается двумя "}"
		ТекстДок.ЗаменитьСтроку(СтрокаВставки ,"}," + Символы.ПС + СтрДобавить);
		Счетчик = Число(Сред(ТекстДок.ПолучитьСтроку(2), 2, 1));
